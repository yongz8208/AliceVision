...
aliceVision::EstimationStatus robustRotationEstimationAC(const Mat &x1, const Mat &x2, std::mt19937 &randomNumberGenerator,  Mat3 &R, std::vector<std::size_t> &vec_inliers)
{
    ...
    robustEstimation::Mat3Model model;
    /*const std::pair<double, double> ACRansacOut =*/robustEstimation::ACRANSAC(kernel, randomNumberGenerator, vec_inliers, 10000, &model, std::numeric_limits<double>::infinity());
    R = model.getMatrix();
